...
relocated Origin, wrapped in EventParameters by default so it can be fed
straight to scdispatch (use --no-wrap for a bare <Origin> instead).
...
Each <in-dir>/<name>.xml -> <out-dir>/<name>.origin.xml (an
EventParameters document by default; a bare <Origin> with --no-wrap).
...
def relocate_bytes(input_xml_bytes, wrap_eventparameters=False):
    """
    ...
    def _fake_write_output(origin, wrap_eventparameters=wrap_eventparameters):
        captured["bytes"] = plx.origin_to_bytes(
            origin, wrap_eventparameters=wrap_eventparameters)
...
def main():
    ...
    ap.add_argument("--no-wrap", dest="wrap_eventparameters",
                    action="store_false",
                    help="emit each per-event output as a BARE <Origin> "
                         "(the shape scolv's LocExt reader produces) instead "
                         "of an EventParameters document. The default is to "
                         "wrap, since batch output is meant for re-import via "
                         "scdispatch, which needs the wrapper. Use --no-wrap "
                         "only if a downstream tool specifically wants the "
                         "bare Origin, or to byte-compare against live scolv "
                         "output.")
    ap.set_defaults(wrap_eventparameters=True)
    args = ap.parse_args()
    ...
    if args.wrap_eventparameters:
        # so the plugin's own main() parses the same value and forwards it
        # to write_output (which we override, but it passes the kwarg through)
        plugin_argv += ["--wrap-eventparameters"]
    ...
    for path in files:
        ...
        try:
            ...
            origin_bytes = relocate_bytes(
                data, wrap_eventparameters=args.wrap_eventparameters)
